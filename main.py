...
intro_message = (
    ...
    "**فاكتبْ، فإنّ روحَ القُدُsِ معك، ما نصرتَ الحق، وأقمتَ الكلمةَ، وجعلتَ المِدادَ جـ ـهـ ادًا.**\n\n"
    "**📚 استكشف أرشيف المادة القيّمة بالأسفل 👇**"
)
...
def send_file(client, callback_query, file_path, caption):
    ...
    try:
        if not os.path.exists(file_path):
            error_msg = f"❌ **خطأ:** لم يتم العثور على الملف في المسار:\n`{file_path}`"
            print(error_msg)
        ...
        error_msg = f"⚠️ **حدث خطأ أثناء الإرسال:**\n`{e}`"
        print(error_msg)
        callback_query.answer(error_msg, show_alert=True)
...
def start(client, message: Message):
    message.reply_text(
        ...
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("📚 استكشاف الأرشيف", callback_data="show_archive")]
        ])
    )
...
def show_archive(client, callback_query):
    keyboard = [
        ...
        [InlineKeyboardButton("🖋️ أبو خيثمة الشنقـ يطي", callback_data="show_abu_khithama")],
        ...
        [InlineKeyboardButton("✍️ مـ يـسـ رة الغـ ريـ ب", callback_data="show_mysara_gharib_books")],
    ...
    callback_query.message.edit_text("📂 **اختر قسم الأرشيف:**", reply_markup=InlineKeyboardMarkup(keyboard))
...
def show_osama_poems(client, callback_query):
    if not poems:
        callback_query.answer("⚠️ **عذراً، لا توجد قصائد متاحة حالياً.**", show_alert=True)
        return

    keyboard = [[InlineKeyboardButton(f"📜 {p['title']}", callback_data=f"poem_{i}")] for i, p in enumerate(poems)]
    keyboard.append([InlineKeyboardButton("⬅️ رجوع", callback_data="show_archive")])
    callback_query.message.edit_text(
        "📜 **قائمة قصائد الشيخ أسامة بن لادن:**",
...
def show_abu_hamza_books(client, callback_query):
    keyboard = [
        [InlineKeyboardButton("📚 هموم وآلام", callback_data="send_abu_hamza_homoom_w_alam")],
...
def show_mysara_gharib_books(client, callback_query):
    keyboard = [
        [InlineKeyboardButton("📝 رمزيات", callback_data="send_mysara_ramziyat")],
        [InlineKeyboardButton("📖 إنما شفاء العي السؤال", callback_data="send_mysara_shifaa_alayi")],
        [InlineKeyboardButton("📒 الكُرَبُ وسُبُلُ تَفْرِيجِها", callback_data="send_mysara_kurab")],
        ...
        [InlineKeyboardButton("🗣️ قـالـوا.. فـقـل!", callback_data="send_mysara_qalou_faqal")],
        [InlineKeyboardButton("⬅️ رجوع", callback_data="show_archive")]
    ]
    callback_query.message.edit_text("✍️ **اختر كتاباً لـ مـ يـسـ رة الغـ ريـ ب:**", reply_markup=InlineKeyboardMarkup(keyboard))
...
def send_mysara_book(client, callback_query):
    book_map = {
        ...
        "send_mysara_qalou_faqal": ("ميسرة الغريب/قـالـوا.. فـقـل!.pdf", "🗣️ **كتاب: قـالـوا.. فـقـل!**"),
    }
...
AHLAM_ALNASER_BOOKS_MAP = {
    ...
    "send_ahlam_alnaser_book_9": ("أوار الحق/9 أتينا لنبقى.. وإن بلغت القلوب الحناجر!.pdf", "🌸 9 أتينا لنبقى.."),
    "send_ahlam_alnaser_book_10": ("أوار الحق/10 منشورات في التربية.pdf", "🌸 10 منشورات في التربية"),
    "send_ahlam_alnaser_book_11": ("أوار الحق/11 إنَّني بريئةٌ منكَ.pdf", "🌸 11 إنَّني بريئةٌ منكَ"),
    "send_ahlam_alnaser_book_12": ("أوار الحق/12 ديوان أوار الحق لأحلام النصر.pdf", "🌸 12 ديوان أوار الحق"),
    "send_ahlam_alnaser_book_13": ("أوار الحق/13 ديوان هدير المعامع لأحلام النصر.pdf", "🌸 13 ديوان هدير المعامع"),
    "send_ahlam_alnaser_book_14": ("أوار الحق/14 أفيـون السهولة، لأحلام النصر.pdf", "🌸 14 أفيـون السهولة"),
    "send_ahlam_alnaser_book_15": ("أوار الحق/15 رحلة علم وجهاد؛ سيرة المجاهد أبي أسامة الغريب.pdf", "🌸 15 رحلة علم وجهاد"),
    "send_ahlam_alnaser_book_16": ("أوار الحق/16 الغلاة.. وبقرة بني إسرائيل!.pdf", "🌸 16 الغلاة.. وبقرة بني إسرائيل!"),
    "send_ahlam_alnaser_book_17": ("أوار الحق/17 وِجاءُ_الثغور_في_دفع_شرور_الكَفور.pdf", "🌸 17 وِجاءُ الثغور"),
    "send_ahlam_alnaser_book_18": ("أوار الحق/18 ديوان سحابة نقاء، لأحلام النصر.pdf", "🌸 18 ديوان سحابة نقاء"),
    ...
    "send_ahlam_alnaser_book_21": ("أوار الحق/21 ربعي بن عامر؛ بين شرعة الله تعالى وشرعة الأمم المتحدة.pdf", "🌸 21 ربعي بن عامر"),
    ...
    "send_ahlam_alnaser_book_27": ("أوار الحق/27 طالبان_على_خطى_مرسي_بقلم_أحلام_النصر.pdf", "🌸 27 طالبان على خطى مرسي"),
    "send_ahlam_alnaser_book_28": ("أوار الحق/28 ليكون الدين كله لله، بقلم أحلام النصر.pdf", "🌸 28 ليكون الدين كله لله"),
    "send_ahlam_alnaser_book_29": ("أوار الحق/29 الجانب التعليمي، أحلام النصر.pdf", "🌸 29 الجانب التعليمي"),
    "send_ahlam_alnaser_book_30": ("أوار الحق/30 أمة الإسناد، لأحلام النصر.pdf", "🌸 30 أمة الإسناد"),
    ...
    "send_ahlam_alnaser_book_اثبت_ولا_تتردد": ("أوار الحق/اثبت_ولا_تتردد،_وبايع_الهزبر_لترشَد (2).pdf", "🌸 اثبت ولا تتردد، وبايع الهزبر لترشَد"),
    ...
    "send_ahlam_alnaser_book_الموت_الزؤام": ("أوار الحق/الموت_الزؤام_لأعداء_نبي_الإسلام_وشعر_أتجرؤون_بقلم_أحلام_النصر.pdf", "🌸 الموت الزؤام لأعداء نبي الإسلام"),
    ...
    "send_ahlam_alnaser_book_لا_يصح": ("أوار الحق/لا يصح إلا الصحيح، والمرتد لن يستريح.pdf", "🌸 لا يصح إلا الصحيح"),
    "send_ahlam_alnaser_book_taysir_altaalim_1": ("أوار الحق/تيسير_التعليم_لمريد_قراءات_القرآن_الكريم_1.pdf", "🌸 تيسير التعليم لمريد قراءات القرآن 1"),
...
def show_ahlam_alnaser_books(client, callback_query):
    # This keyboard can be dynamically generated from the map for easier maintenance
    keyboard = [
        [InlineKeyboardButton(v[1], k)] for k, v in AHLAM_ALNASER_BOOKS_MAP.items() if k.startswith("send_ahlam_alnaser_book_")
    ]
    keyboard.append([InlineKeyboardButton("📚 قصة: عائد من الظلام (كل الأجزاء)", callback_data="show_aed_min_althalam_parts")])
    keyboard.append([InlineKeyboardButton("⬅️ رجوع", callback_data="show_archive")])
    
    callback_query.message.edit_text(
        "🌸 **اختر من مؤلفات أحلام النصر الدمشقية:**",
        reply_markup=InlineKeyboardMarkup(keyboard)
    )
