...
def show_poem(client, callback_query):
    try:
        ...
        if 0 <= idx < len(current_poems):
            ...
            print(f"✅ تم إرسال قصيدة: {title} للمستخدم {callback_query.message.chat.id}")
        else:
            print(f"❌ مؤشر خارج النطاق: {idx} >= {len(current_poems)}")
