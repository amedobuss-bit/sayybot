...
if not BOT_TOKEN:
    print("Error: TG_BOT_TOKEN is required. Please set it as environment variable or in config.ini")
    # إنشاء تطبيق فارغ لتجنب الأخطاء
    app = Flask(__name__)
    @app.route('/')
    def error_page():
        return "Error: TG_BOT_TOKEN is required", 500
    application = app
else:
